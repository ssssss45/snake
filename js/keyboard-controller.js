...
//обработчик начала касания
	touchClickStart(event)
	{
		...
		if (event.type=="touchstart")
		{
			...
			var touch = new CustomEvent("controls:touch", {
					detail: {
								touch:event.changedTouches[0],
							}
					});
				var elem = this.getElement();
				elem.dispatchEvent(touch);
		}
		if (event.type=="mousedown")
		{
			...
			var click = new CustomEvent("controls:click", {
					detail: {
								click:event,
							}
					});
...
//Обработчик события конца кaсания и генератор соответствующих событий
	...
	touchMove(event)
	{
		var swipeEvent = new CustomEvent("controls:touch moved", {
		detail: {
					touch:event.changedTouches[0]
				}
		});
...
//Нацеливает контроллер на переданный DOM-елемент (вешает слушатели).
	attach(target){
		...
		elem.addEventListener("touchstart",this.boundTouchClickStart,false);
		elem.addEventListener("touchend",this.boundTouchClickEnd,false);
		elem.addEventListener("mousedown",this.boundTouchClickStart,false);
		elem.addEventListener("mouseup",this.boundTouchClickEnd,false);
		elem.addEventListener("touchmove",this.boundTouchMove,false);
    }
...
//Отцепляет контроллер от активного DOM-елемента и деактивирует контроллер.
	detach(){
		...
		elem.removeEventListener("touchstart",this.boundTouchClickStart,false);
		elem.removeEventListener("touchend",this.boundTouchClickEnd,false);
		elem.removeEventListener("mousedown",this.boundTouchClickStart,false);
		elem.removeEventListener("mouseup",this.boundTouchClickEnd,false);
		elem.removeEventListener("touchmove",this.boundTouchMove,false);
	}
...
//Проверяет нажата ли переданная кнопка в контроллере
	...
	addKeyToController(name,keys)
	{
		...
		this.bindActions(key);
	}

	addTouchToController(name,keys)
	{
		var key={
			name: name,
			coords: keys,
